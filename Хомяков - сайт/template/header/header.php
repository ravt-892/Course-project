<html>
    <head>
        <meta charset="UTF-8">
        <link rel="stylesheet" href="/template/css/style.css">
        <link rel="icon" href="/template/images/favicon.ico" type="image/x-icon">
        <title>Course</title>
        
    </head>
    <body>
        <div class="container2">
<a class="toggleMenu" href="#">Menu</a>
<ul class="nav">
    <li><a href=".." class="test">Мой Course</a></li>
    <li><a>Фильмы</a>
        <ul>
            <li><a href="#">Артхаус</a></li>
            <li><a href="#">Боевики</a></li>
            <li><a href="#">Военные</a></li>
			<li><a href="#">Детективы</a></li>
			<li><a href="#">Для всей семьи</a></li>
			<li><a href="#">Для детей</a></li>
			<li><a href="#">Документальные</a></li>
			<li><a href="#">Драмы</a></li>
			<li><a href="#">Исторические</a></li>
			<li><a href=#>Комедии</a></li>
			<li><a href=#>Криминальные</a></li>
			<li><a href=#>Мелодрамы</a></li>
			<li><a href=#>Мистические</a></li>
			<li><a href=#>Приключения</a></li>
			<li><a href=#>Катастрофы</a></li>
			<li><a href=#>Триллеры</a></li>
			<li><a href=#>Ужасы</a></li>
			<li><a href=#>Фантастика</a></li>
			<li><a href=#>Фэнтези</a></li>
			<li><a href=#>Эротика</a></li>
        </ul>
    </li>
    
    <li><a>Сериалы</a>
        <ul>
            <li><a href="#">Боевики</a></li>
            <li><a href="#">Военные</a></li>
            <li><a href="#">Детективы</a></li>
            <li><a href="#">Драмы</a></li>
            <li><a href="#">Исторические</a></li>
            <li><a href="#">Комедийные</a></li>
            <li><a href="#">Криминальные</a></li>
            <li><a href="#">Мелодрамы</a></li>
            <li><a href="#">Приключения</a></li>
            <li><a href="#">Романтические</a></li>
            <li><a href="#">Триллеры</a></li>
            <li><a  href="#">Фантастические</a></li>
        </ul>
    </li>
    
    <li><a>Мультфильмы</a>
        <ul>
            <li><a  href="#">Советские</a></li>
            <li><a  href="#">Аниме</a></li>
            <li><a  href="#">Для взрослых</a></li>
            <li><a  href="#">Для детей</a></li>
            <li><a  href="#">Полнометражные</a></li>
            <li><a  href="#">Приключения</a></li>
            <li><a  href="#">Развивающие</a></li>
            <li><a  href="#">Сериалы</a></li>
        </ul>
    </li>
    
	
	<li><a  href="#">Тут будет поиск</a></li>
    <li><a  href="/login" id="simple">Войти</a></li>
    <li><a  href="/signup">Регистрация</a></li>
</ul>
</div>

<script type="text/javascript" src="http://code.jquery.com/jquery-1.7.2.min.js"></script>
<script type="text/javascript" src="/template/header/script.js"></script>

    </body>
</html>